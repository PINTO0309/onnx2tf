...
def test_primary_path_retains_terminal_swish_qdq_island_result() -> None:
    ...
    assert len(statement.targets) == 1
    assert isinstance(statement.targets[0], ast.Name)
    assert statement.targets[0].id == "_terminal_swish_qdq_island_stats"
    call = statement.value
    assert isinstance(call, ast.Call)
    assert isinstance(call.func, ast.Name)
    assert call.func.id == callback_name
...
def test_primary_path_retains_terminal_instancenorm_post_bias_result() -> None:
    ...
    assert len(statement.targets) == 1
    assert isinstance(statement.targets[0], ast.Name)
    assert statement.targets[0].id == "_terminal_instancenorm_post_bias_stats"
    call = statement.value
    assert isinstance(call, ast.Call)
    assert isinstance(call.func, ast.Name)
    assert call.func.id == callback_name
    ...
    assert len(predecessor.targets) == 1
    assert isinstance(predecessor.targets[0], ast.Name)
    assert predecessor.targets[0].id == "_terminal_swish_qdq_island_stats"
...
def test_primary_path_retains_very_late_instancenorm_post_bias_result() -> None:
    ...
    assert terminal.targets[0].id == "_terminal_instancenorm_post_bias_stats"
...
@pytest.mark.xfail(
    strict=True,
    reason="terminal InstanceNorm residual-add result is not retained",
)
...
def test_primary_path_retains_very_late_instancenorm_dualstats_result() -> None:
    ...
    assert isinstance(terminal, ast.Assign)
    assert isinstance(terminal.targets[0], ast.Name)
    assert terminal.targets[0].id == "_terminal_instancenorm_dualstats_stats"
...
def test_primary_path_retains_terminal_instancenorm_dualstats_result() -> None:
    ...
    assert len(statement.targets) == 1
    assert isinstance(statement.targets[0], ast.Name)
    assert statement.targets[0].id == "_terminal_instancenorm_dualstats_stats"
    call = statement.value
    assert isinstance(call, ast.Call)
    assert isinstance(call.func, ast.Name)
    assert call.func.id == callback_name
