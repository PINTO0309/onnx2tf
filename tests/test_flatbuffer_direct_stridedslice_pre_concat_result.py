...
OWNER_NAME = "optimize_transpose_stridedslice_pre_concat_nhwc_chains"
RESULT_TARGET = "_layout_opt_stridedslice_pre_concat_stats"
PREVIOUS_TARGET = "_layout_opt_ndhwc_concat_stats"
FOLLOWING_TARGET = "_layout_opt_split_mixed_pre_concat_stats"
...
def test_stridedslice_pre_concat_schema_cleanup_and_selection_are_explicit() -> None:
    wrapper = _functions(LOWERER_PATH)[STRIDED_SLICE_PRE_CONCAT]
    ...
    assert wrapper_return.value.func.id == f"{STRIDED_SLICE_PRE_CONCAT}_pass"
    ...
    assert not any(
        isinstance(node, ast.Name) and node.id == STRIDED_SLICE_PRE_CONCAT
        for node in ast.walk(orchestration)
    )


def test_stridedslice_pre_concat_zero_counter_can_prune_unused_tensor() -> None:
    model_ir = ModelIR("stridedslice_pre_concat_zero_prune")
    model_ir.inputs = ["x"]
    model_ir.outputs = ["x"]
    ...
    assert stats == {
        "optimized_transpose_stridedslice_pre_concat_nhwc_chains": 0,
    }
...
def test_direct_stridedslice_pre_concat_result_is_retained_observation_only() -> None:
    ...
    layout_guard = next(
        ...
        and any(
            _call_name(child) == STRIDED_SLICE_PRE_CONCAT
    ...
    result_index = next(
        ...
        if _call_name(statement) == STRIDED_SLICE_PRE_CONCAT
    )
    ...
    assert sum(
        ...
        and node.func.id == STRIDED_SLICE_PRE_CONCAT
    ) == 1
    assert not any(
