...
def test_indexed_flatten_hw_wrapper_prunes_and_tracks_layout() -> None:
    ...
    assert optimize_transpose_reshape_transpose_to_flatten_hw_nhwc_chains_compat(
        compat_ir,
    ...
    assert set(wrapper_ir.tensors) == {"x", "shape", "z"}
...
def test_indexed_flatten_hw_preserves_dynamic_compatibility_fallback() -> None:
    ...
    assert _fingerprint(compat_ir) == _fingerprint(wrapper_ir)
    assert [operator.op_type for operator in wrapper_ir.operators] == ["RESHAPE"]
