...
@pytest.mark.parametrize("semantic_rank", [4, 5])
def test_conv3d_gate_layout_runner_reuses_one_index(
    semantic_rank: int,
    ...
    stats_key = "optimized_transpose_conv3d_leaky_mul_unsqueeze_ndhwc_chains"
    assert stats[stats_key] == 1
    assert refresh_count == 1
    assert len(diagnostics) == 2
    assert diagnostics[1]["code"] == "layout.ndhwc_conv3d_leaky_unsqueeze_gate"
    assert diagnostics[1]["changed"] is True
    assert sum(
        int(event["metrics"]["snapshot_count"]) for event in diagnostics
    ) == 1
...
def test_conv3d_gate_layout_runner_rejects_before_snapshot(
    boundary: str,
) -> None:
    model_ir = _model(boundary=boundary)
    diagnostics: list[dict[str, object]] = []
    ...
    assert all(event["skipped_by_precondition"] is True for event in diagnostics)
    assert all(
        event["metrics"]["snapshot_count"] == 0 for event in diagnostics
    )
