...
def test_layout_pass_set_1_cleanup_results_are_guarded_and_unconsumed() -> None:
    lowerer = _lowerer()
...
@pytest.mark.xfail(
    strict=True,
    reason="layout pass-set 1 cleanup results have not moved to phase records",
)
def test_layout_pass_set_1_cleanup_results_use_phase_result_store() -> None:
    lowerer = _lowerer()
    guard = _guard(lowerer)
    records = [
        statement
