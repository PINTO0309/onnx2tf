...
SUCCESSOR_TARGET = "_final_slice_concat_recovery_results"
OLD_RESULT_TARGETS = (
...
def test_final_boundary_channel_cluster_is_ordered_and_unconsumed() -> None:
    lowerer = _lowerer()
    indices: list[int] = []
    expected_keywords = (
        {
...
@pytest.mark.xfail(
    strict=True,
    reason="final boundary channel cluster has not moved to one composite owner",
)
