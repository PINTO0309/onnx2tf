...
def test_concat_unary_conv_layout_runner_reuses_one_index(
    ...
    stats = run_concat_unary_conv_layout_cleanup(
        model_ir,
    ...
    assert diagnostics[0]["code"] == "layout.concat_unary_conv_nhwc"
...
def test_concat_unary_conv_layout_runner_rejects_before_snapshot(
    boundary: str,
) -> None:
    model_ir = _model(boundary=boundary)
    original = deepcopy(model_ir)
