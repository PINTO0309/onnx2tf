...
OWNER = "run_layout_pass_set_1_mean_attention_gate_cleanup"
CHILD_OWNERS = (
    "run_mean_attention",
    "run_attention_gate_qdq_recovery",
)
...
COMPOSITE_TARGET = "_layout_pass_set_1_mean_attention_gate_results"
PREDECESSOR_PHASE_ID = "cleanup.layout_pass_set_1.mean_affine_prepost"
SUCCESSOR_PHASE_ID = "cleanup.layout_pass_set_1.quantized_prelu"
...
def _context() -> tuple[ModelIRPassContext, AttentionRecoveryContext]:
    model_ir = ModelIR("layout_pass_set_1_mean_attention_gate_schema")
    pass_context = ModelIRPassContext(
        model_ir=model_ir,
        layout_state=LayoutState.from_model_ir(model_ir),
        diagnostics=[],
    )
    recovery_context = AttentionRecoveryContext(
    ...
    return pass_context, recovery_context
...
def test_layout_pass_set_1_mean_attention_gate_current_contract() -> None:
    ...
    assert ast.unparse(
        context_assignments["mean_attention_context"].value
    ) == "shared_model_ir_pass_context"
    recovery_assignment = context_assignments["attention_recovery_context"]
    assert isinstance(recovery_assignment.value, ast.Call)
    assert {
