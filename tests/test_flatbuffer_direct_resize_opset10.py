...
def _make_opset9_linear_upsample_model() -> onnx.ModelProto:
    image = helper.make_tensor_value_info(
        "image",
        TensorProto.FLOAT,
        [1, 1, 4, 4],
...
def test_opset9_linear_upsample_uses_half_pixel_coordinates(tmp_path: Path) -> None:
    model = _make_opset9_linear_upsample_model()
    model_ir = lower_onnx_to_ir(
        onnx_graph=model,
        output_file_name="opset9_linear_upsample",
        allow_custom_ops=False,
        optimize_layout_transpose_chains=False,
    )
    resize_op = next(
        op for op in model_ir.operators if str(op.op_type) == "RESIZE_BILINEAR"
    )
    assert resize_op.options["alignCorners"] is False
    assert resize_op.options["halfPixelCenters"] is True
