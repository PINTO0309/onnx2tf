...
`ModelIRPassContext` and the unchanged `include_layout_transpose` value first
to terminal QKV shape/attention cleanup and then to terminal activation-bridge
cleanup. The owner returns the complete two-result QKV tuple and three-result
activation tuple as one ordered outer tuple. It does not copy, flatten,
normalize, inspect, or use either child result as control flow.

The lowerer replaces only the two unconsumed observation locals with
`_terminal_qkv_activation_bridge_results` and removes the now-unused direct
child imports. The pre-terminal affine/Slice/SPP composite remains the
immediate predecessor, and terminal layout/shape remains the immediate
successor. Specialized child owners, raw compatibility wrappers, callbacks,
independent QKV and activation routes, graph mutations, and layout policy are
unchanged.

Owner-aware QKV, Shape-extract, indexed Split/Conv/Concat, HardSwish-SE, late
hard-activation, adjacent-composite, and architecture tests now resolve the
top-level call through the new owner while continuing to inspect the original
child owners for their internal contracts. Runtime monkeypatch injection for
both layout-option values proves exact two-child order, shared-context
identity, exact keyword forwarding, and identity of both raw nested result
tuples.

Final sequential validation under core-only `uv` passed with 5 focused tests,
535 affected tests, 92 terminal-layout/efficiency tests, 55 core tests, 196
result-contract tests, 2 phase-store tests, and 11 TensorFlow import-blocking,
default-direct, and `-cotof` tests. Ruff, bytecode compilation, and whitespace
checks also pass. No pass invocation, graph rewrite, callback, guard, phase
result, public API, artifact, dependency, or TensorFlow boundary changed. The
bounded store remains exactly 128 IDs and 128 owners. The unconsumed lowerer
assignment inventory decreases from 60 to 59. No real-model conversion was
...
## Extract the layout-pass-set-1 QLinear/attention boundary

`passes/layout_pass_set_1_qlinear_attention_recovery_orchestration.py` now
owns the characterized pair. It forwards the exact embedded
`ModelIRPassContext` to public QLinear/Mean/Concat recovery, then forwards the
original callback-bearing `LayoutRecoveryContext` to public final
layout/reshape/attention recovery. Both complete nested tuples are returned in
...
dequant-Mean and InstanceNorm phases remain the immediate boundaries. Both
lowerer compatibility wrappers, the independent layout-pass-set-2 QLinear and
layout-recovery-prefix routes, and every callback path remain available.

Ten stale structural assertions now follow the outer owner while continuing
to verify the independent direct routes. Sequential validation passes:
focused `3`, affected `394`, and standard `92 / 55 / 196 / 2 / 11`. Ruff,
...
## Extract late affine/final-shape/terminal cleanup

`passes/late_affine_final_shape_terminal_orchestration.py` now owns the two
characterized composites. It forwards the existing late-boundary context's
exact `ModelIRPassContext` to late affine/optional fan-out cleanup, then
forwards the original late-boundary context to final-shape/terminal cleanup.
The same layout option is passed to both children, and both complete result
objects are returned by identity.

The lowerer replaces only `_late_affine_optional_fanout_results` and
`_late_final_shape_terminal_fanout_results` with
`_late_affine_final_shape_terminal_results`. Recorded late NDHWC cost-volume
remains the predecessor and the terminal Conv/Pool/no-layout branch remains
the successor. Both child owners, wrappers, graph behavior, public APIs,
artifacts, dependencies, and TensorFlow isolation remain unchanged; only the
two unused direct child imports are removed from the lowerer.

The first affected run deliberately recorded 52 failures across 26 files; all
were stale direct-owner, target, route, or neighbor expectations caused by the
ownership move. After owner-aware updates, sequential validation passes:
focused `6`, affected `433`, and standard `92 / 55 / 196 / 2 / 11`. Ruff,
bytecode compilation, and whitespace checks pass. The phase store remains
exactly 128 IDs and 128 owners, while the managed unconsumed-result inventory
