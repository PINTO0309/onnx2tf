...
The twelfth through sixteenth families are the separate direct, unary,
Pad-plus-direct, unary-plus-Pad, and all-Pad quantized-post passes
...
`layout.nhwc_pre_concat_quantized_all_pad`, in
...
complete expanded-Swish diamond, exact Pad, bounded direct-source Slice, or
bounded Split. Exact Pad and Slice plans may also accompany the Add graph at
the root Concat. Every Add
...
one-time materialization. Each nested inverse post adapter is
...
and Slice output fan-out reject before mutation. Dequantize/PReLU/Softmax and
other mixed operand families deliberately remain in legacy.
...
The direct/unary/Pad/unary-plus-Pad/all-Pad quantized-post families validate
...
materialized and reused. Public boundaries, invalid ranks/spatial metadata,
and non-Transpose fan-out reject transactionally. Other broader mixed
quantized inputs continue through legacy.
...
skips the sixteen indexed families, but continues to own broader
...
- Direct, unary, Pad, Dequantize, PReLU, Softmax, expanded-Swish,
  ...
  modules; authoritative collection now contains 199. Including the bounded
  direct and unary/Pad quantized-post suites, the compact inventory contains
  247 tests across nine modules. The preceding combined run passed 208 tests;
  ...
  output-post-adapter bypass, nineteen complete no-op boundaries, and one
...
Next work should characterize a bounded Dequantize Add operand, or the next
