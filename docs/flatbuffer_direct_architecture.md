...
direct-adapter, unary, and Pad-plus-direct families are owned by
...
`layout.nhwc_pre_concat_unary`, and `layout.nhwc_pre_concat_pad`. Exclusive
pads constants are remapped in place; shared or public pads constants use
copy-on-write so unrelated Pad consumers preserve NCHW semantics. Canonical
Concat, unary, and Pad per-axis quantization metadata remap NCHW dimension 1
to NHWC dimension 3. Swish, split, slice, Add, PReLU, Dequantize, Softmax, and
quantized-post families remain in the legacy matcher until each receives
