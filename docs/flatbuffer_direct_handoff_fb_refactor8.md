...
## Layout-pass-set-2 QLinear/layout implementation checkpoint

`passes/layout_pass_set_2_qlinear_layout_recovery_orchestration.py` now
provides `run_layout_pass_set_2_qlinear_layout_recovery()`. It calls
`run_qlinear_mean_concat_recovery(context.pass_context)` and then
`run_layout_recovery_prefix(context)`, returning both raw tuples unchanged.
Runtime injection proves exact child order, pass-context and callback-bearing
context identity, absence of extra options, and both result identities.

The lowerer replaces only `_layout_pass_set_2_qlinear_mean_concat_results` and
`_layout_pass_set_2_layout_recovery_prefix_results` with
`_layout_pass_set_2_qlinear_layout_recovery_results`. The existing
`_set_post_progress_desc("layout recovery pass-set 2")` call remains the direct
predecessor and `_layout_pass_set_2_preadd_mean_attention_results` remains the
direct successor. Both compatibility wrappers and every set-1, nested
attention, callback, and downstream route remain available.

Eight stale direct-call or neighbor expectations were updated to follow the
public children through the new owner while retaining total route coverage.
Sequential validation passes: focused `6`, complete affected `392`,
terminal-layout/efficiency `92`, core `55`, result contracts `196`, phase store
`2`, and TensorFlow import-blocking/default-direct/`-cotof` `11`. Ruff,
bytecode compilation, and whitespace checks pass. The phase-result store
remains exactly 128 IDs and 128 owners, and the read-only unconsumed-result
inventory decreases from 43 to 42. No real-model conversion was repeated for
this straight-line ownership-only extraction.

At resume, refresh the 42-result inventory and select the next smallest
source-adjacent, semantically closed observation-only boundary. Characterize
all guards, phase or progress boundaries, option policies, exact context and
...
## Layout-pass-set-2 channel/pre-add characterization checkpoint

The refreshed inventory selects `_layout_opt_channel_shuffle_gather_results`
followed immediately by `_layout_opt_preadd_mean_attention_results`. Both are
unconditional inside the second `optimize_layout_transpose_chains` guard. The
first wrapper uses `channel_shuffle_gather_context`, which aliases
`session.model_ir_pass_context`, and enables only
`include_post_gather_cleanup=True`; the second wrapper uses
`attention_recovery_context`, whose `pass_context` is that exact session
context.

The characterization fixes the complete seven-slot channel and seven-slot
pre-add schemas, nested mean-attention callback identity, source adjacency,
option policy, observation-only results, and both compatibility wrappers. The
recorded `cleanup.layout_pass_set_2.slice_logistic_concat_tail` phase is the
predecessor; recorded `cleanup.layout_pass_set_2.sa_pa_mirrorpad` is the
successor.

Production is unchanged pending
`passes/layout_pass_set_2_channel_preadd_orchestration.py` and
`run_layout_pass_set_2_channel_preadd_recovery()`. That owner must call
`run_channel_shuffle_gather()` with the exact `context.pass_context` and
`include_post_gather_cleanup=True`, then call
`run_preadd_mean_attention_recovery(context)`, returning both complete tuples
in source order and by identity. The lowerer must replace only the two selected
locals with `_layout_pass_set_2_channel_preadd_results`, passing the original
`attention_recovery_context` and preserving both recorded phase boundaries.

Focused sequential validation reports `2 passed, 1 xfailed`; complete
reference-based affected validation reports `424 passed, 1 xfailed`. The sole
expected failure requires the intentionally absent owner. Production, the
41-result inventory, and the exactly 128-ID/128-owner phase store are
unchanged. No real-model conversion was repeated for this characterization.

At resume, implement only this characterized owner and lowerer replacement,
convert the strict xfail to runtime order/context/option/result-identity
coverage, and update only structural expectations made stale by the new outer
...
## Late affine/final-shape/terminal characterization checkpoint

The managed inventory contains 32 unconsumed lowerer results after excluding
the two intentionally retained layout-pass-set-1 recovery-prefix observations.
The selected boundary consists of two adjacent unconditional assignments:
`_late_affine_optional_fanout_results` from
`run_late_affine_optional_fanout_cleanup(shared_model_ir_pass_context,
include_elementwise_fanout=optimize_layout_transpose_chains)`, then
`_late_final_shape_terminal_fanout_results` from
`run_late_final_shape_terminal_fanout_cleanup(
late_final_shape_boundary_context,
include_elementwise_fanout=optimize_layout_transpose_chains)`.

`late_final_shape_boundary_context.pass_context` is the exact
`shared_model_ir_pass_context`. The characterization fixes that identity,
complete affine/Concat, final-shape/boundary, optional fan-out, and singleton
schemas, exact child order, and both Boolean option paths. Both results remain
observation-only. Recorded `cleanup.late.ndhwc_cost_volume` stays the direct
predecessor, while the terminal Conv/Pool/no-layout branch stays the direct
successor and remains outside the extraction.

Production remains unchanged pending
`passes/late_affine_final_shape_terminal_orchestration.py` and
`run_late_affine_final_shape_terminal_cleanup()`. The owner must call
`run_late_affine_optional_fanout_cleanup(context.pass_context,
include_elementwise_fanout=include_elementwise_fanout)` followed by
`run_late_final_shape_terminal_fanout_cleanup(context,
include_elementwise_fanout=include_elementwise_fanout)` and return both raw
results by identity. The lowerer must replace only the two selected targets
with `_late_affine_final_shape_terminal_results`, pass the exact existing late
context and layout option, retain both child owners and independent routes,
and preserve both surrounding boundaries.

Focused sequential validation reports `3 passed, 1 xfailed`; the fixed
27-file affected suite reports `430 passed, 1 xfailed`. The sole expected
failure requires the intentionally absent owner. Phase-store validation is
`2 passed`; production, the managed 32-result inventory, and the exactly
128-ID/128-owner store remain unchanged. Ruff, bytecode compilation, and
whitespace checks pass. No real-model conversion was repeated for this
characterization.
...
by the new outer route. Run the affected suite once before changing stale
tests, then run all affected and standard gates sequentially under `uv`,
confirm the expected managed inventory reduction from 32 to 31, and commit and
push the complete unit. Do not create, update, reopen, or otherwise modify a
pull request.

## Late affine/final-shape/terminal implementation checkpoint

`passes/late_affine_final_shape_terminal_orchestration.py` now provides
`run_late_affine_final_shape_terminal_cleanup()`. It calls
`run_late_affine_optional_fanout_cleanup(context.pass_context,
include_elementwise_fanout=include_elementwise_fanout)` followed by
`run_late_final_shape_terminal_fanout_cleanup(context,
include_elementwise_fanout=include_elementwise_fanout)`. It returns both raw
child results without copying, flattening, schema inspection, or result-driven
control flow. Runtime injection for both Boolean paths proves exact order,
context identity, option forwarding, and result identity.

The lowerer replaces `_late_affine_optional_fanout_results` and
`_late_final_shape_terminal_fanout_results` with
`_late_affine_final_shape_terminal_results`. It passes the existing
`late_final_shape_boundary_context` and unchanged layout option. Recorded
`cleanup.late.ndhwc_cost_volume` remains the direct predecessor and the
terminal Conv/Pool/no-layout branch remains the direct successor. The branch
and its recorded no-layout phase stay outside the owner. Both child owners,
nested lowerer wrappers, callbacks, and independent routes remain available.
Only the two now-unused direct child-runner imports were removed.

The first fixed 27-file affected run was intentionally executed before test
updates and recorded 52 failures across 26 files. Every failure was a stale
direct-call, target, route, or neighbor assertion caused by the ownership move;
no runtime schema, option path, context identity, pass behavior, phase store,
or TensorFlow boundary failed. Those structural expectations now follow both
children through the new outer owner.

Sequential validation passes: focused `6`, fixed 27-file affected `433`,
terminal-layout/efficiency `92`, core `55`, result contracts `196`, phase
store `2`, and TensorFlow import-blocking, default-direct, and `-cotof` `11`.
Ruff, bytecode compilation, and whitespace checks pass. The read-only AST
audit reports 33 raw unconsumed results, 31 managed results after the two
intentionally retained layout-pass-set-1 recovery-prefix observations, zero
old selected targets, one new composite target, and exactly 128 phase IDs with
128 owners. No real-model conversion was repeated for this ownership-only
extraction.

At resume, refresh the managed 31-result inventory and select the next
smallest source-adjacent, semantically closed observation-only boundary.
Do not merge the mutually exclusive terminal Conv/Pool and no-layout affine
branches across the recorded no-layout phase. Characterize all guards,
recorded phase boundaries, option policies, exact context and callback
