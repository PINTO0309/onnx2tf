    ...
    if 'is_instance_norm_3d' in tf_layers_dict[graph_node_input_1.name] \
        ...
        try:
            ...
            if two_previous_op_type == 'Reshape' and one_previous_op_type == 'Transpose':
                two_previous_op_output_rank = len(two_previous_op.outputs[0].shape)
                one_previous_op_output_rank = len(one_previous_op.outputs[0].shape)
                current_op_output_rank = len(graph_node.outputs[0].shape)
