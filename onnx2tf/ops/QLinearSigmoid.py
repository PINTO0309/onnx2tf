...
def make_node(
    ...
    **kwargs: dict,
    ...
    dequantized_sigmoid = tf.nn.sigmoid(
            x=dequantized_x,
            name=graph_node.name,
        )
    dequantized_sigmoid = tf.add(
        x=tf.divide(
            x=dequantized_sigmoid,
            y=y_scale,
        ),
        y=y_zero_point,
    )

    casted_sigmoid = tf.cast(dequantized_sigmoid, output_dtype)
