...
_SQUEEZE_AXIS_NCHW = 2
_SQUEEZE_AXIS_NHWC = 1
...
class _Plan:
    ...
    squeeze_output: str
    squeeze_view: _View
    mapped_squeeze_options: Tuple[Tuple[str, Any], ...]
    post: OperatorIR
    suffix_output: str
    suffix_view: _View
...
def _static_view(view: _View, rank: int) -> bool:
    return bool(
...
def _mapped_squeeze_options(squeeze: OperatorIR) -> Optional[Tuple[Tuple[str, Any], ...]]:
    if not isinstance(squeeze.options, dict):
        return None
    options = dict(squeeze.options)
    ...
    try:
        ...
        return None
    if axes != (_SQUEEZE_AXIS_NCHW,):
        return None
    options["squeezeDims"] = [_SQUEEZE_AXIS_NHWC]
...
def _resolve_candidate(
    ...
    *,
    ...
    for candidate_logistic_input_index in (0, 1):
        ...
        candidate_pre_output = str(mul.inputs[int(candidate_data_input_index)])
        ...
        if (
            ...
            or str(candidate_logistic.inputs[0]) != candidate_pre_output
        ...
        pre_output = candidate_pre_output
    ...
    pre_index = graph_index.producers.get(pre_output)
    if (
        pre_index is None
        or int(pre_index) >= int(logistic_index)
        or int(pre_index) >= int(mul_index)
        or pre_output in graph_index.duplicate_producers
        or pre_output in public_names
        or logistic_output in public_names
        or mul_output in public_names
    ):
        return None
    pre = model_ir.operators[int(pre_index)]
    if (
        graph_index.producer(pre_output) is not pre
        ...
        or str(pre.outputs[0]) != pre_output
        or not _exclusive_slot_consumers(
            model_ir,
            graph_index,
            pre_output,
    ...
    if source_name in graph_outputs:
        return None

    squeeze_users = graph_index.consumers_of(mul_output)
    if len(squeeze_users) != 1:
        return None
    squeeze = squeeze_users[0]
    squeeze_index = _operator_index(graph_index, squeeze)
    mapped_options = _mapped_squeeze_options(squeeze)
    if (
        squeeze_index is None
        or int(squeeze_index) <= int(mul_index)
        or _op_type(squeeze) != "SQUEEZE"
        or len(squeeze.inputs) != 1
        or len(squeeze.outputs) != 1
        or str(squeeze.inputs[0]) != mul_output
        or mapped_options is None
        or not _exclusive_slot_consumers(
            model_ir,
            graph_index,
            mul_output,
        ...
        return None
    squeeze_output = str(squeeze.outputs[0])
    if squeeze_output in public_names or squeeze_output in graph_index.duplicate_producers:
        return None
    post_users = graph_index.consumers_of(squeeze_output)
    if len(post_users) != 1:
        return None
    post = post_users[0]
    ...
    if (
        post_index is None
        or int(post_index) <= int(squeeze_index)
        or not _typed_rank3_permutation(model_ir, graph_index, post)
        or str(post.inputs[0]) != squeeze_output
        or not _exclusive_slot_consumers(
            model_ir,
            graph_index,
            squeeze_output,
    ...
    if (
        suffix_output in graph_index.duplicate_producers
        or graph_index.producer(suffix_output) is not post
    ):
        return None
    ...
    squeeze_tensor = model_ir.tensors.get(squeeze_output)
    ...
    assert squeeze_tensor is not None
    ...
    if (
        nchw_view is None
        or not _static_view(source_view, 4)
        or not _static_view(nchw_view, 4)
        or int(nchw_view.shape[_SQUEEZE_AXIS_NCHW]) != 1
        or int(nchw_view.signature[_SQUEEZE_AXIS_NCHW]) != 1
    ):
        return None
    squeeze_view = _View(
        shape=(nchw_view.shape[0], nchw_view.shape[1], nchw_view.shape[3]),
        signature=(
            nchw_view.signature[0],
            nchw_view.signature[1],
            nchw_view.signature[3],
        ),
        dtype=nchw_view.dtype,
    )
    suffix_view = _View(
        shape=(squeeze_view.shape[0], squeeze_view.shape[2], squeeze_view.shape[1]),
        signature=(
            squeeze_view.signature[0],
            squeeze_view.signature[2],
            squeeze_view.signature[1],
        ),
        dtype=squeeze_view.dtype,
    )
    if (
        ...
        or _view(squeeze_tensor) != squeeze_view
    ...
    involved_operators = (pre, logistic, mul, squeeze, post)
    involved_names = {
        source_name,
        pre_output,
        str(pre.inputs[1]),
    ...
    return _Plan(
        ...
        squeeze_view=squeeze_view,
        mapped_squeeze_options=mapped_options,
        post=post,
...
def _apply_plan(
    ...
    *,
    ...
    for name in (plan.logistic_output, plan.mul_output):
        ...
        _set_layout(tensor, name, LOGICAL_LAYOUT_NHWC, layout_state)
    _set_layout(
    ...
    squeeze_options = dict(plan.squeeze.options)
    squeeze_options["squeezeDims"] = [_SQUEEZE_AXIS_NHWC]
    plan.squeeze.options = squeeze_options
    _set_operator_outputs(
        model_ir=model_ir,
        op=plan.squeeze,
    ...
    suffix_tensor.quantization = _clone_quantization(
        model_ir.tensors[plan.squeeze_output].quantization
    )
...
def optimize_transpose_pre_swish_squeeze_transpose_suffix_nhwc_chains(
    model_ir: ModelIR,
    *,
    ...
    """Index the strict static Swish/axis-2 Squeeze suffix family."""
