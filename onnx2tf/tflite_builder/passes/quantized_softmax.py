...
def _tensor_metadata(
    ...
    try:
        ...
        return None
    if len(shape) == 0 or len(shape) != len(signature):
        return None
    return shape, signature


def _has_supported_quantized_softmax_contract(
...
def _has_supported_softmax_options(
    options: object,
    *,
    rank: int,
) -> bool:
    if not isinstance(options, dict) or int(rank) <= 0:
        return False
    try:
        beta = float(options.get("beta", 1.0))
        axis = int(options.get("axis", int(rank) - 1))
    except (TypeError, ValueError, OverflowError):
        return False
    if not np.isfinite(beta) or not np.isclose(
        beta,
        1.0,
        rtol=0.0,
        atol=1e-6,
        ...
        return False
    normalized_axis = axis + int(rank) if axis < 0 else axis
    return normalized_axis == int(rank) - 1
...
def _optimize_dequant_softmax_quantize_chains(
    ...
    *,
    ...
    """Fold a canonical last-axis DQ→Softmax→Q chain."""

    stats_key = "folded_dequant_softmax_quantize_chains"
    active_index = (
        graph_index
    ...
    if active_index is None:
        required_types = {"DEQUANTIZE", "SOFTMAX", "QUANTIZE"}
        ...
        for operator_type in ("DEQUANTIZE", "SOFTMAX", "QUANTIZE")
    ):
        _prune_unused_tensors(model_ir, layout_state=layout_state)
        return {stats_key: 0}
    ...
    while True:
        ...
        for dequantize_index in active_index.operator_indices("DEQUANTIZE"):
            ...
            if len(float_input_consumers) != 1:
                continue
            softmax_index = int(float_input_consumers[0])
            if int(dequantize_index) >= softmax_index:
                continue
            softmax = model_ir.operators[softmax_index]
            if (
                str(softmax.op_type) != "SOFTMAX"
                or len(softmax.inputs) != 1
                or len(softmax.outputs) != 1
                or str(softmax.inputs[0]) != float_input_name
            ):
                continue

            float_output_name = str(softmax.outputs[0])
            if float_output_name in public_boundaries:
                continue
            if float_output_name in active_index.duplicate_producers:
                continue
            if active_index.producers.get(float_output_name) != softmax_index:
                continue
            ...
            if softmax_index >= quantize_index:
                continue
            ...
            if (
                ...
                or not _has_supported_quantized_softmax_contract(
                    quantized_input,
                    quantized_output,
                )
            ):
            ...
            if not _has_supported_softmax_options(
                softmax.options,
                rank=len(output_shape),
            ):
                continue

            # All grid, metadata, option, topology, and boundary guards are
            # complete. Retain Softmax options and public output identity.
            _set_operator_inputs(
                model_ir=model_ir,
                op=softmax,
            ...
            softmax.version = 2 if str(quantized_output.dtype).upper() == "INT8" else 1
